import { Command, START, StateGraph } from "@langchain/langgraph";
import { DaiaLanggraphStateMachine } from "../machine/machine";
import { DaiaLanggraphMachineNode } from "../machine/machineDefines";
import { DaiaLanggraphNamespacedState, DaiaLanggraphNamespacedStateSchema } from "../state";

/**
 * Config, which configures DAIA graph, which in turn can be used as subgraph in order to provide more features with DAIA.
 */
export type DaiaGraphConfig = {
	/**
	 * Public key to send when remote DAIA agent requests public key from this agent.
	 */
	publicKey: string;

	/**
	 * Maps node names from internal DAIA langgraph representation to actual node names used by langgraph.
	 */
	mapNode: (node: DaiaLanggraphMachineNode) => string;
};

export const makeDaiaGraph = <T extends DaiaLanggraphNamespacedState>(config: DaiaGraphConfig) => {
	const machine = new DaiaLanggraphStateMachine({
		publicKey: config.publicKey,
	});

	const graph = new StateGraph(DaiaLanggraphNamespacedStateSchema as any)
		.addNode("input", async (state: T) => {
			const output = await machine.run(state.daia);

			return new Command({
				goto: config.mapNode(output.targetNode),
				graph: Command.PARENT,
				update: {
					daia: output.newState,
				},
			});
		})
		.addEdge(START, "input");


	return graph.compile();
};
