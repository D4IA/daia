...
const MenuIcon = ({ src, alt }: { src: string; alt: string }) => <img src={src} alt={alt} />;
...
const LanguageIconRenderer: React.FC<{ code: string }> = ({ code }) => {
	...
	if (code === "pz") {
		return <img src={icon} alt={code.toUpperCase() + " logo"} className={styles.langIcon} />;
	}

	return (
		<span className={styles.langEmoji} role="img" aria-label={code.toUpperCase()}>
			{icon}
		</span>
	);
};
...
const Navbar: React.FC = () => {
	...
	const displayLangCode = AVAILABLE_LANGUAGES.find((lang) => lang.code === currentLangCode)
		? currentLangCode
	...
	const LanguageDropdown: React.FC = () => (
		<div className={styles.langDropdownContainer}>
			<button
				...
				title={t(
					AVAILABLE_LANGUAGES.find((l) => l.code === displayLangCode)?.labelKey || "language_names.en",
				)}
			>
	...
	return (
		<div className={styles.navbarContainer}>
			...
			<div className={`${styles.mobileMenu} ${isMenuOpen ? styles.mobileMenuOpen : ""}`}>
				{!hideSearch && (
					<form onSubmit={handleSubmit} className={styles.mobileSearchContainer}>
						<input
							type="text"
							placeholder={t("navbar.button")}
				...
				<Link to="/developers" className={styles.mobileNavLink} onClick={closeMenu}>
					{t("navbar.developers")}
				</Link>
				<Link to="/about_us" className={styles.mobileNavLink} onClick={closeMenu}>
					{t("navbar.about_us")}
				</Link>
