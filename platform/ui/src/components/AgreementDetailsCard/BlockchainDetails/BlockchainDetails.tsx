...
const TransactionStatus: React.FC<TransactionStatusProps> = ({ status, transactionHash }) => {
		...
		<div className={styles.offerDetails}>
			<div className={styles.row}>
				...
				<div className={`${styles.statusDisplay} ${statusInfo.colorClass}`}>
					<img src={statusInfo.icon} alt={status} className={styles.statusIcon} />
...
const OfferDetailsCard: React.FC<OfferDetailsCardProps> = ({ title, offer }) => {
	return (
		<div className={styles.offerCardWrapper}>
			<h4 className={styles.offerTitle}>{title}</h4>
			<TransactionStatus status={offer.status} transactionHash={offer.transactionHash} />
		</div>
	);
};

const BlockchainDetails: React.FC<BlockchainDetailsProps> = ({ proposerOffer, responderOffer }) => {
	return (
