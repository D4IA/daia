import { DaiaAgreementReferenceResult, DaiaAgreementVerificationResult, DaiaOfferBuilder } from "@daia/core";
import {
...
import { GateEnterAgentStateSchema } from "./state";
...
export function createGateAgentGraph(adapter: GateAgentEnterAdapter) {
    ...
    const graph = new StateGraph(GateEnterAgentStateSchema)
        ...
        .addNode(
            ...
            {
                ends: [sendDaiaOutput, continueConversation, afterRemoteProcessedLocalOffer, afterOfferReceived],
            },
        )
        .addNode(afterOfferReceived, async (state) => {
            ...
            return produce(state, draft => {
                ...
                draft.daia = DaiaLanggraphStateWriter.fromState(state.daia).setOfferResponse({
        ...
        .addNode(afterRemoteProcessedLocalOffer, async (state) => {
            ...
            if (response.result === DaiaAgreementReferenceResult.ACCEPT) {
                ...
                const verificationResult = await adapter.getVerifier().getAgreementFromTransaction(agreementReference)

                if (!verificationResult.found || verificationResult.verification.result !== DaiaAgreementVerificationResult.PASSED) {
                ...
                return produce(state, draft => {
                    ...
                    draft.conversationHistory.push(
                        { role: "user" as const, content: "Offer was rejected: " + (response.rationale ?? "no rationale provided") },
        ...
        .addNode(continueConversation, async (state) => {
            ...
            if (assistantResponse.type === "offer") {
                ...
                const offer = DaiaOfferBuilder.new()
                    .setNaturalLanguageContent(assistantResponse.offer)
                ...
                return new Command({
                    ...
                    update: {
                        ...
                        conversationHistory: [
                            ...state.conversationHistory,
                            { role: "user" as const, content: input },
                            { role: "assistant" as const, content: assistantResponse.offer },
                        ],
                    },
                });
            }
